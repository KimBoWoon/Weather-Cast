...
//    val weatherInfoList = MutableLiveData<List<WeatherInfo?>>()
    ...
    val status = MutableStateFlow<Status>(Status.Loading)
    val geocoding = mutableListOf<Geocoding>(
        Geocoding(37.62f, 55.75f),
        Geocoding(35f, 139f),
...
//                weatherInfoList = weatherList
                ...
                status.value = Status.Success(weatherList)
            }.onFailure { e ->
                status.value = Status.Failure(e.message ?: "something error")
            }
        }
    }
}